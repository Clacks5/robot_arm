...
def main():
	...
	try:    # try内で何らかのエラーが発生 -> 処理中断してexceptに移動
		...
		r0 = np.array([150, -100, 50])
		r1 = np.array([100, -150, 50])

		R = np.array([r0, r0 + np.array([0, 0, 100]), r1 + np.array([0, 0, 100]), r1]).T
		print(R)
		...
		for i in range(4):
			J = calc_inverse_kinematics(R[0][i], R[1][i], R[2][i])
			for j in range(6):                  # 6つの角度値を表示
				print(f"J = {j+1}: {J[j]}")

			if move_mode == 2:
				mycobot.set_color(color[i][0], color[i][1], color[i][2])
			moveto(J=J, marker_pos=[R[0][i], R[1][i], R[2][i]])
...
def calc_inverse_kinematics(x, y, z):
	...
	if y > 0:
		theta[0] = np.pi - np.arctan2(x, y) - np.arccos(d4 / np.sqrt(x*x + y*y))
	else:
		theta[0] = -(np.pi/2 - np.arctan2(x, -y) - np.arcsin(d4 / np.sqrt(x*x + y*y)))
	
	X = (x - d5*np.cos(theta[0]) - d4*np.sin(theta[0])) / np.cos(theta[0])
	Z = z + d6 - d1

	sqrt_check(1 - ((X*X + Z*Z - a2*a2 - a3*a3)/(2*a2*a3))**2)
	theta[2] = -np.arctan2(np.sqrt(1 - ((X*X + Z*Z - a2*a2 - a3*a3)/(2*a2*a3))**2), (X*X + Z*Z - a2*a2 - a3*a3)/(2*a2*a3))

	alpha = np.arctan2(Z, X)
	beta = np.arctan2(a3*np.sin(theta[2]), a3*np.cos(theta[2]) + a2)
	theta[1] = alpha - beta
